...
static struct ndpi_proto packet_processing(struct ndpi_workflow * workflow,
					   const u_int64_t times,
					   u_int16_t vlan_id,
					   const struct ndpi_iphdr *iph,
					   struct ndpi_ipv6hdr *iph6,
					   u_int16_t ip_offset,
					   u_int16_t ipsize, u_int16_t rawsize,struct reader_thread ndpi_thread_info,u_int16_t thread_id) {
...
if(flow != NULL){
    ...
    //strftime(MY_TIME, sizeof(MY_TIME), "%d/%m/%Y - %I:%M:%S %p", tmp);
    //printf("Formatted date & time : %s\n", MY_TIME );
    strftime(our_time, sizeof(our_time), "%H:%M:%S", tmp);
    strftime(our_date, sizeof(our_date), "%d-%m-%Y", tmp);
    //printf("Time: %s\n", our_time);
    //printf("Date: %s\n", our_date);
    json_object_object_add(jObject,"threadid",json_object_new_int(thread_id));
    json_object_object_add(jObject,"packetNumber",json_object_new_int(count));
    json_object_object_add(jObject,"Time",json_object_new_string(our_time));
    json_object_object_add(jObject,"Date",json_object_new_string(our_date));
    json_object_object_add(jObject,"protocol",json_object_new_string(ipProto2Name(flow->protocol)));
    json_object_object_add(jObject,"host_a_name",json_object_new_string(flow->src_name));
    json_object_object_add(jObject,"host_a_port",json_object_new_int(ntohs(flow->src_port)));
    json_object_object_add(jObject,"host_b_name",json_object_new_string(flow->dst_name));
    json_object_object_add(jObject,"host_b_port",json_object_new_int(ntohs(flow->dst_port)));
    
    if(flow->detected_protocol.master_protocol)
      json_object_object_add(jObject,"detected_master_protocol",json_object_new_int(flow->detected_protocol.master_protocol));

    json_object_object_add(jObject,"detected_app_protocol",json_object_new_int(flow->detected_protocol.app_protocol));
    ...
    if(flow->detected_protocol.master_protocol) {
      ...
      snprintf(tmp, sizeof(tmp), "%s.%s",
	       ndpi_get_proto_name(ndpi_thread_info.workflow->ndpi_struct, flow->detected_protocol.master_protocol),
	       ndpi_get_proto_name(ndpi_thread_info.workflow->ndpi_struct, flow->detected_protocol.app_protocol));

      json_object_object_add(jObject,"detected_protocol_name",
			     json_object_new_string(tmp));

      
    } else
      json_object_object_add(jObject,"detected_protocol_name",
			     json_object_new_string(ndpi_get_proto_name(ndpi_thread_info.workflow->ndpi_struct,
									flow->detected_protocol.app_protocol)));



    json_object_object_add(jObject,"bytes",json_object_new_int(flow->src2dst_bytes + flow->dst2src_bytes));

    if(flow->host_server_name[0] != '\0')
      json_object_object_add(jObject,"host_server_name",json_object_new_string(flow->host_server_name));
    ...
    if((flow->ssh_ssl.client_info[0] != '\0') || (flow->ssh_ssl.server_info[0] != '\0')) 
{
	...
	json_object_object_add(sjObj, "server", json_object_new_string(flow->ssh_ssl.server_info));
      ...
      json_object_object_add(jObject, "ssh_ssl", sjObj);
   ...
   /*koky*/  
    ...
    json_string=json_object_to_json_string(jObject);
    
    int len=strlen(json_string);
    int num_of_spaces=400-len-1;

    strcpy(temp,json_string);
    memset(temp+len,'k',num_of_spaces);
    temp[len+num_of_spaces]='\0';
    printf("%s\n",json_string);
    printf("%d\n",fd[id]);

    
    returnValue=write(fd[id], temp, 400);
    printf("%d\n",returnValue);
    
    
   
    //printf("%s\nlength:%d \nspaces:%d \n",temp,len,num_of_spaces);
...
struct ndpi_proto ndpi_workflow_process_packet (struct ndpi_workflow * workflow,
						...
						const u_char *packet, struct reader_thread ndpi_thread_info,u_int16_t thread_id) {
  /*
...
iph_check:
  ...
  return(packet_processing(workflow, time, vlan_id, iph, iph6,
			   ip_offset, header->caplen - ip_offset, header->caplen, ndpi_thread_info,thread_id));
}
