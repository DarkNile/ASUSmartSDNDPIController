...
static void pcap_process_packet(u_char *args,
  ...
  p = ndpi_workflow_process_packet(ndpi_thread_info[thread_id].workflow, header, packet_checked, ndpi_thread_info[thread_id],thread_id);
...
/*koky*/
  ...
  fd[id] = socket(AF_INET, SOCK_STREAM, 0);
